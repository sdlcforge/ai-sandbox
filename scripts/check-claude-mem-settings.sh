...
if [ -z "$CURRENT_HOST" ]; then
    ...
    echo "Adding CLAUDE_MEM_WORKER_HOST=0.0.0.0 for ai-container compatibility..."
    jq '. + {"CLAUDE_MEM_WORKER_HOST": "0.0.0.0"}' "$SETTINGS_FILE" > "${SETTINGS_FILE}.tmp"
    mv "${SETTINGS_FILE}.tmp" "$SETTINGS_FILE"
    echo "Done."
elif [ "$CURRENT_HOST" = "0.0.0.0" ]; then
    echo "claude-mem is already configured for ai-container (CLAUDE_MEM_WORKER_HOST=0.0.0.0)"
elif [ "$CURRENT_HOST" = "127.0.0.1" ]; then
    echo "Updating CLAUDE_MEM_WORKER_HOST from 127.0.0.1 to 0.0.0.0 for ai-container compatibility..."
    jq '.CLAUDE_MEM_WORKER_HOST = "0.0.0.0"' "$SETTINGS_FILE" > "${SETTINGS_FILE}.tmp"
    mv "${SETTINGS_FILE}.tmp" "$SETTINGS_FILE"
