...
Usage:
  ...
  ai-sandbox                             Enter the default sandbox instance
  ai-sandbox ls                          List all sandboxes (instances and profiles)

Global commands:
  instances ls                       List all managed sandbox instances.
  instances create <name> [options]  Create and start a new sandbox instance. Options:
                                       ...
                                       --allow-egress <host-or-ip-or-cidr>:<port>  Allow outbound
                                                          traffic to an additional host/IP/CIDR on the
                                                          given port. Repeatable.
  ...
  profiles ls                        List all available profiles.
  profiles create <name> [options]   Scaffold a new profile YAML by auto-discovering skills,
                                      hooks, and agents. Options:
                                       --mode <mode>      mirror or static (default: mirror).
                                       --output <path>    Write to <path> instead of the default location.
                                       --plugins <names>  Comma-separated plugin names to include.
...
Per-instance commands (ai-sandbox <name> <command>):
  ...
  delete             Remove the container (docker compose down). For a name that
                     resolves to a profile instead, removes its YAML file (e.g.
                     'ai-sandbox <profile-name> delete').
  attach             Attach a shell to an already-running container.
  detail             Show container state, built images, and runability. For
                     a name that resolves to a profile instead, prints its
                     raw YAML content.
