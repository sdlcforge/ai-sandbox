...
Single-session plan. Phase 01 (Full Config Restore) begins first; its three tasks are sequential (Task 002 depends on the label schema and shares `src/utils.sh`/the test file with Task 001; Task 003 depends on the `ai.sandbox.config` label Task 001 introduces and is sequenced after Task 002 to avoid `test/unit/ai_sandbox_spec.sh` merge friction). Phase 02 (Documentation Updates) runs after Phase 01 lands. Pre-condition: dependencies are not installed in the plan worktree, but implementation happens on task branches where `make build`/`make lint`/`make test.unit` are available. No blocking pre-conditions.

## Overview

### Phase 01 — Full Config Restore

Implements the persistence/restore contract from the design note. Three sequential tasks (not parallel-eligible): Task 002 shares `src/utils.sh`/the test file with Task 001 and depends on the label schema Task 001 defines; Task 003 depends on the `ai.sandbox.config` label Task 001 introduces and is sequenced after Task 002 purely to avoid `test/unit/ai_sandbox_spec.sh` merge friction (no functional dependency on Task 002's content):
