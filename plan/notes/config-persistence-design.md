...
- **No backward-compatibility fallback (reversed by explicit product decision).** Containers created before this change carry no `ai.sandbox.config` label. Per explicit product decision, `restore_saved_config` reads **only** the new `ai.sandbox.config` label — no fallback to the legacy `ai.sandbox.profiles`/`ai.sandbox.mode`/`ai.sandbox.clean-slate` labels is implemented. Rationale: there are no external users of this tool yet, and a single label-based config regime is preferred over supporting two in parallel. The practical consequence: a container missing `ai.sandbox.config` (including any created before this change) simply restores nothing extra on a bare `enter`/`start` — equivalent to today's default un-configured behavior (mirror mode, no clean-slate, no profiles). This is an accepted non-goal, not a gap to guard against.
...
**Recommended design:** a single base64-encoded JSON label `ai.sandbox.config` holding the complete seven-dimension config-input record, written at create/recreate time by `index.sh` + `docker-compose.yaml`; read by an extended `restore_saved_config` to rehydrate all inputs on a bare `enter`/`start`; complemented by an extended `running_config_matches` that compares the full derived-config dimension set (adding marketplaces/plugins/enable-all via three new plain derived labels). The existing plain labels are retained for `list` display and matches.
