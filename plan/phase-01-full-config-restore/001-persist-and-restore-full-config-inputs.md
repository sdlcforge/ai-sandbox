...
Implement the input-persistence half of the design note (§2.1, §2.2, §2.3 restore side, §2.5 backward-compat):
...
3. **Extend `restore_saved_config` in `src/utils.sh`** to rehydrate all seven inputs, keeping the existing `CONFIG_FLAGS_PROVIDED != true` && `is_container_running_or_stopped` gate:
   ...
   - **Backward-compatibility fallback (hard requirement, design note §2.5):** when `ai.sandbox.config` is absent or empty (containers created before this change), fall back to the current behavior — read the legacy `ai.sandbox.profiles`/`ai.sandbox.mode`/`ai.sandbox.clean-slate` labels exactly as today. Pre-existing containers must not regress.
...
- [config persistence design recommendation](../notes/config-persistence-design.md) — authoritative design; §2.1 (shape), §2.2 (write timing), §2.3 (restore side), §2.5 (base64 + backward-compat), §2.6 (label inventory).
...
- After extending `restore_saved_config` with the JSON read + legacy fallback (read side complete).
